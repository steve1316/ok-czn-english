...
GENERATED_NOTE = "Generated by `scripts/build_game_data.py` - do not edit by hand. Re-run it after a game patch."
...
def collect_combatants(table, dump_dir):
    ...
    """
    in_dump = {name for name, _ in named_combatants(table, dump_dir)}
...
def render_tables(summary, notes, tables, comments, scalars=None):
    """Build the source of a generated module holding constant tables.

    Args:
        summary: The first line of the module docstring.
        notes: Further docstring lines saying what the module does and does not carry.
        tables: A dict of constant name to the mapping or set it holds.
        comments: The line to write above each constant.
        scalars: Single values to write ahead of the tables, for a figure the runtime must agree with.

    Returns:
        The module source.
    """
    lines = ['"""' + summary, "", GENERATED_NOTE, "", *notes, "",
             SOURCE_NOTE, f"Generated: {date.today().isoformat()}", '"""', ""]
...
def named_combatants(table, dump_dir):
    """Walk the combatant table, yielding the combatants a player can actually field.

    Test units and placeholder names are skipped, so every caller agrees on who counts as a combatant rather
    ...
    Yields:
        A `(name, row)` pair for each real combatant.
    ...
    for row in json.loads(path.read_text(encoding="utf-8")):
        if str(row.get("id", "")).startswith(TEST_ID_PREFIX):
            continue
        name = (table.get(COMBATANT_NAME_ID.format(id=row.get("id"))) or "").strip()
...
def linked_id(row, column):
    """Read an id one row links to, in the form the dump writes them.

    Args:
        row: The data row.
        column: The column holding the link.

    Returns:
        The id, empty when the column holds none.
    """
...
def collect_battle(table, dump_dir):
    ...
    combatant_ego_card = {}
    combatant_ego_cost = {}
    for combatant, row in named_combatants(table, dump_dir):
        attribute = attributes.get(linked_id(row, EGO_TYPE_COLUMN))
        if attribute:
            combatant_attribute[combatant] = attribute
        sparked = [name_by_id.get(linked_id(row, column)) for column in COMBATANT_SPARK_COLUMNS]
        started = [name_by_id.get(linked_id(row, column)) for column in COMBATANT_START_COLUMNS]
        for card in filter(None, started + sparked):
        ...
        ego = all_rows.get(linked_id(row, COMBATANT_EGO_COLUMN))
        if ego:
        ...
        unique = tuple(sorted(set(filter(None, sparked))))
        if unique:
            combatant_spark_cards[combatant] = unique
...
def render_battle(battle):
    ...
    """
    comments = {
        "X_COST": "What a card with no fixed price reads as: it spends every Action Point left. Written here"
                  " rather than in the reader, so the two cannot drift apart.",
        "ATTRIBUTES": "Every attribute, keyed by the colour the client stores it as, which is the colour it is"
                      " drawn in as well.",
        "CARD_COST": "What a card costs to play, in Action Points.",
    ...
    return render_tables("What the client knows about playing a card, taken from its own data tables.",
                         notes, battle, comments, scalars={"X_COST": X_COST})
...
def main():
    ...
    OUT_QUALITY_PATH.write_text(render_quality(quality), encoding="utf-8")
    OUT_BATTLE_PATH.write_text(render_battle(battle), encoding="utf-8")
