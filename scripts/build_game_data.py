...
OUT_BATTLE_PATH = REPO_ROOT / "src" / "en" / "game_battle.py"
...
# The columns on a player card row that a battle decision needs.
CARD_COST_COLUMN = "cost"
CARD_CATEGORY_COLUMN = "card_category"
CARD_TARGET_COLUMN = "target_for_ui"
CARD_SPARK_COLUMN = "battle_r_spark_enable_flag"
CARD_SORT_COLUMN = "sort"
# Kinds of card that share the deck's tables but never reach a hand. A Persona sits in a slot of its own and
# an Ego skill is fired from the combatant bar, so neither one's `cost` is an Action Point cost. Leaving them
# in would put an Ego skill's EP price of 5 beside a card that really does cost 5 Action Points.
NON_HAND_SORTS = frozenset({"SORT_SUPPORT", "SORT_COLLAPSE_SKILL"})
# The combatant's Ego skill. The client calls it a collapse skill in the data and an Ego skill on screen.
COMBATANT_EGO_COLUMN = "collapse_skill_link_card_id"
# A negative cost means the card spends every Action Point left rather than a fixed number. The client
# writes it as -1 or -2, and the difference does not change how a card is chosen, so both collapse to one.
X_COST = -1
# A combatant owns eight cards: the four they start a run holding, and the four an Epiphany can hand them.
# This is the card-to-combatant link the card table itself does not carry.
COMBATANT_START_COLUMNS = tuple(f"start_{index}_link_card_id" for index in (1, 2, 3, 4))
COMBATANT_SPARK_COLUMNS = tuple(f"spark_card_{index}_link_card_id" for index in (1, 2, 3, 4))
# A combatant's attribute is stored as a colour and named in a table of its own, so both halves are read
# from the dump rather than written down here and left to drift.
EGO_TYPE_TABLE = "ego_type@ego_type.json"
EGO_TYPE_COLUMN = "link_ego_type_id"
# The five colours the game shows a player. The table also holds internal rows for monsters and for "same
# attribute as the holder", which are not attributes anyone picks between, and this flag is how it says so.
EGO_TYPE_PLAYER_FLAG = "enable_show_archive_filter"
...
def cost_of(value):
    """Read a card's Action Point cost.

    Args:
        value: The raw `cost` column, which the dump writes as a string.

    Returns:
        The cost, `X_COST` when the card spends everything left, or None when the column does not parse.
    """
    try:
        cost = int(str(value).strip())
    except (TypeError, ValueError):
        return None
    return X_COST if cost < 0 else cost


def collect_attributes(table, dump_dir):
    """Pair each attribute colour with the name the game prints for it.
    ...
    Returns:
        A dict of colour id to attribute name, covering only the attributes a player is shown.

    Raises:
        SystemExit: When the attribute table is missing.
    """
    path = Path(dump_dir) / "db" / EGO_TYPE_TABLE
    if not path.exists():
        raise SystemExit(f"no attribute table at {path}")

    attributes = {}
    for row in json.loads(path.read_text(encoding="utf-8")):
        if row.get(EGO_TYPE_PLAYER_FLAG) != "YES":
            continue
        name = (table.get((row.get("title") or "").strip()) or "").strip()
        if name:
            attributes[str(row.get("id"))] = name
    return attributes


def collect_battle(table, dump_dir):
    """Pair every card and combatant with what deciding a turn needs to know about them.
    ...
    Returns:
        A dict of constant name to value, ready to render.
    """
    all_rows = load_data_tables(dump_dir, PLAYER_CARD_TABLES)
    # Personas and Ego skills are dropped before anything reads a cost, so no EP price is mistaken for an
    # Action Point one. Combatants still reach their own Ego through `all_rows`, which keeps them.
    rows = {row_id: row for row_id, row in all_rows.items() if row.get(CARD_SORT_COLUMN) not in NON_HAND_SORTS}
    named = base_rows_by_name(rows, table)
    # A combatant reaches its cards by id, so the reverse of the name lookup is needed too. Upgrade tiers share
    # a name and `base_rows_by_name` keeps the shortest id, so only those ids are mapped back here. Agreeing
    # with it means a combatant's card names the same row every other generated table describes.
    name_by_id = {}
    for row_id, row in rows.items():
        name = table.get(row.get("name") or "")
        if name in named:
    ...
    for name, row in named.items():
        cost = cost_of(row.get(CARD_COST_COLUMN))
        if cost is not None:
            card_cost[name] = cost
        category = CATEGORIES.get(row.get(CARD_CATEGORY_COLUMN))
        if category:
            card_category[name] = category
        target = str(row.get(CARD_TARGET_COLUMN) or "").strip()
        if target and target != "none":
            card_target[name] = target
        if row.get(CARD_SPARK_COLUMN) == "YES":
            card_sparkable.add(name)

    attributes = collect_attributes(table, dump_dir)
    roster = set(PLAYABLE_COMBATANTS)
    card_owner = {}
    combatant_attribute = {}
    ...
    for row in json.loads(Path(dump_dir, "db", COMBATANT_TABLE).read_text(encoding="utf-8")):
        ...
        if combatant_id.startswith(TEST_ID_PREFIX):
            continue
        combatant = (table.get(COMBATANT_NAME_ID.format(id=combatant_id)) or "").strip()
        if not combatant or JUNK.match(combatant):
            continue
        attribute = attributes.get(str(row.get(EGO_TYPE_COLUMN) or "").strip())
        if attribute:
            combatant_attribute[combatant] = attribute
        for column in COMBATANT_START_COLUMNS + COMBATANT_SPARK_COLUMNS:
            card = name_by_id.get(str(row.get(column) or "").strip())
            if not card:
                continue
            # A few cards are listed by two combatants, one of whom has not been released. The released one is
            # whose attribute the card carries in a real run, so they take it. Otherwise the first claim keeps it.
            held = card_owner.get(card)
            if held is None or (held not in roster and combatant in roster):
                card_owner[card] = combatant
        ego = all_rows.get(str(row.get(COMBATANT_EGO_COLUMN) or "").strip())
        if ego:
            ego_name = (table.get(ego.get("name") or "") or "").strip()
            ego_cost = cost_of(ego.get(CARD_COST_COLUMN))
            if ego_name:
                combatant_ego_card[combatant] = ego_name
            if ego_cost is not None:
                combatant_ego_cost[combatant] = ego_cost
        sparked = (name_by_id.get(str(row.get(column) or "").strip()) for column in COMBATANT_SPARK_COLUMNS)
        sparked = tuple(sorted({card for card in sparked if card}))
        if sparked:
            combatant_spark_cards[combatant] = sparked
    ...
    return {
        ...
        "CARD_SPARKABLE": card_sparkable,
        "CARD_OWNER": card_owner,
        "COMBATANT_ATTRIBUTE": combatant_attribute,
        "COMBATANT_SPARK_CARDS": combatant_spark_cards,
        "COMBATANT_EGO_CARD": combatant_ego_card,
        "COMBATANT_EGO_COST": combatant_ego_cost,
    }


def render_battle(battle, dump_dir):
    """Write the module holding what a battle decision needs to know.

    Args:
        battle: The collected mappings.
        dump_dir: Path to the dump, recorded in the header.
    ...
    """
    comments = {
        "ATTRIBUTES": "Every attribute, keyed by the colour the client stores it as, which is the colour it is"
                      " drawn in as well.",
        "CARD_COST": f"What a card costs to play. {X_COST} means it spends every Action Point left rather than a"
                     " fixed number.",
        "CARD_CATEGORY": "What kind of card this is, matching the type label printed under it in hand. A Curse or"
                         " a Status Ailment is something done to you, never something worth playing.",
        "CARD_TARGET": "Who a card is aimed at, which is also how to tell whether playing it asks for a target at"
                       " all.",
        "CARD_SPARKABLE": "The cards that can take an Epiphany, so a glow read off the screen can be checked"
                          " against the card it appears on.",
        "CARD_OWNER": "The combatant a card belongs to, recovered from the cards each of them starts and sparks."
                      " A card's attribute is its owner's, so this is how a card comes to have one.",
        "COMBATANT_ATTRIBUTE": "Each combatant's attribute, for matching a card against the weakness an enemy"
                               " shows.",
        "COMBATANT_SPARK_CARDS": "The unique cards a combatant's own Epiphany hands them.",
        "COMBATANT_EGO_CARD": "The Ego skill each combatant fires from the combatant bar.",
        "COMBATANT_EGO_COST": "What that Ego skill costs. It is spent from the EP bar rather than from Action"
                              " Points, which is why no Ego skill appears in `CARD_COST`.",
    }
    lines = [
    ...
    for name, values in battle.items():
        lines.append("")
        lines.append(f"# {comments[name]}")
        if isinstance(values, dict):
            lines.append(f"{name} = {{")
            lines.extend(f"    {key!r}: {values[key]!r}," for key in sorted(values))
            lines.append("}")
        else:
...
def main():
    ...
    battle = collect_battle(table, args.dump)
    ...
    OUT_BATTLE_PATH.write_text(render_battle(battle, args.dump), encoding="utf-8")
    ...
    print(f"wrote {OUT_BATTLE_PATH.relative_to(REPO_ROOT)} with "
          + ", ".join(f"{len(values)} {name.lower()}" for name, values in battle.items()))
    return 0
